...
import droidninja.filepicker.utils.ScanFileUtil
...
class VMDocPicker(application: Application) : BaseViewModel(application) {
    ...
    suspend fun queryDocs(fileTypes: List<FileType>, storagetype:StorageTypes, comparator: Comparator<Document>?): HashMap<FileType, List<Document>> {
        ...
        withContext(Dispatchers.IO) {
            ...
            if (storagetype == StorageTypes.MANAGE){
            ...
            if (storagetype == StorageTypes.SPECIAL){
                for (exdir in APP_DIR_LIST){
                    ...
                    documents.addAll(getDocumentFromDocumentFiles(files))
                }
                return@withContext
            }
...
//        }
    ...
    @WorkerThread
    private fun getDocumentFromDocumentFiles(data: Array<DocumentFile>): MutableList<Document> {
        val documents = mutableListOf<Document>()
    ...
    @WorkerThread
    private fun getDocumentFromFiles(data: MutableList<File>): MutableList<Document> {
        val documents = mutableListOf<Document>()
        for (fileitem in data){
            val imageId:Long = 0
            val path = fileitem.absolutePath
            val title = fileitem.name

            if (path != null && title != null) {
                val fileType = getFileType(PickerManager.getFileTypes(), path)
                val document = Document(imageId, title, Uri.parse(path))
                document.fileType = fileType
                document.size = fileitem.length().toString()
                document.mtime = (fileitem.lastModified() / 1000).toString()
                document.mimeType = StorageTool.getMimeType(fileitem)
                if (!documents.contains(document)) document?.let { documents.add(it) }
            }
        }
        return documents
    }
    ...
    private suspend fun getAllFiles() : MutableList<File>{
